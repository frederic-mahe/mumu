...
namespace {
  ...
  auto print_log_header(std::ofstream& log_file) noexcept -> void {
    log_file
      ...
      << "shared_incidence" << sepchar  // 10. number of samples where both the potential parent OTU and the query OTU are present
      ...
      << "relative_incidence" << sepchar  // 17. relative incidence (shared incidence / query incidence)
