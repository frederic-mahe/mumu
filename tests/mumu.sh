...
## mumu stops with an error if minimum_ratio is negative
DESCRIPTION="mumu stops with an error if minimum_ratio is negative"
OTU_TABLE=$(mktemp)
MATCH_LIST=$(mktemp)
NEW_OTU_TABLE=$(mktemp)
LOG=$(mktemp)
...
"${MUMU}" \
    ...
    --new_otu_table "${NEW_OTU_TABLE}" \
    --log "${LOG}" \
    --minimum_relative_cooccurrence 0.5 \
    --minimum_relative_cooccurence 0.5 > /dev/null 2>&1 && \
    success "${DESCRIPTION}" || \
        failure "${DESCRIPTION}"
rm -f "${OTU_TABLE}" "${MATCH_LIST}" "${NEW_OTU_TABLE}" "${LOG}"
...
DESCRIPTION="mumu log file header columns are tab-separated"
OTU_TABLE=$(mktemp)
MATCH_LIST=$(mktemp)
NEW_OTU_TABLE=$(mktemp)
LOG=$(mktemp)
"${MUMU}" \
    ...
    --new_otu_table "${NEW_OTU_TABLE}" \
    --log "${LOG}" > /dev/null 2>&1
awk 'BEGIN {FS = "\t"} END {exit NF == 18 ? 0 : 1}' "${LOG}" && \
    success "${DESCRIPTION}" || \
        failure "${DESCRIPTION}"
rm -f "${OTU_TABLE}" "${MATCH_LIST}" "${NEW_OTU_TABLE}" "${LOG}"
...
DESCRIPTION="mumu log file entry columns are tab-separated"
OTU_TABLE=$(mktemp)
MATCH_LIST=$(mktemp)
NEW_OTU_TABLE=$(mktemp)
LOG=$(mktemp)
